...
def detectar_parrafo(linea):
    ...
    print(f"es parrafo? {linea[8:12]} {len(linea[8:12])}")
    if len(linea[8:12]) > 0 and all(c.isalpha() or c in '.-' for c in linea[8:12]):
        if stripped.endswith('.') and not stripped.upper().startswith('PERFORM'):
            posible_parrafo = stripped.rstrip('.')
            posible_parrafo = re.sub(r"\s*SECTION", "", posible_parrafo , flags=re.IGNORECASE)
            print(f"posible parrafo {posible_parrafo}")
            # Excluir ciertas palabras clave que no son nombres de párrafo
            if posible_parrafo.upper() not in ['VARYING', 'UNTIL', 'WITH', 'END-IF', 'END-EXEC', 'STOP RUN', 'EXIT', 'CONTINUE']:
                print(f"segun esto es parrafo -->{linea[8:]}")
...
def generar_grafo(diccionario, selects_por_parrafo, archivo_salida, analizar_sql=False):
    ...
    dot.attr(dpi='300', rankdir='LR', nodesep='1.0', ranksep='1.5', splines='ortho')  # Configuración para orientación, espaciado y aristas ortogonales
    dot.attr('node', shape='box', style='filled', fontname='Helvetica', fontsize='10')
    """
